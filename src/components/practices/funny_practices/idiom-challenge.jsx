...
const IdiomChallenge = () => {
    ...
    const questions = React.useMemo(() => {
        ...
        for (const q of shuffled) {
            if (result.length === 0 || result[result.length - 1].idiom !== q.idiom) result.push(q);
    ...
    return (
        ...
        {selectedAnswer !== null && currentQuestion === questions.length - 1 ? (
            ...
            <div className="flex justify-between mt-16">
                <button onClick={handleReset} className="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 transition">
                <ArrowLeft className="w-5 h-5" /> Back
                </button>
                <Link to="/" className="flex items-center gap-2 px-3 py-2 text-sm text-white bg-[#1E293B] rounded-md shadow-sm transition" style={{ border: "none", color: "white" }}>
            ...
            <div className="flex justify-between items-center mb-4">
                ...
                <span className="text-gray-500 text-sm">{currentQuestion + 1} of {questions.length}</span>
            </div>
            ...
            {selectedAnswer !== null && currentQuestion < questions.length - 1 && (
                ...
                <button onClick={handleNext} className="py-2 px-6 bg-[#1E293B] text-white rounded-md text-lg font-semibold hover:bg-[#111827] transition" style={{ border: "none" }}>
                    Next →
                </button>
                </div>
            )}
