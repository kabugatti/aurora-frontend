...
import {Bell, Menu, Search, Settings, LogIn, LogOut, BookOpen, Lightbulb, Award, User, Bot, X} from "lucide-react";
...
import PracticeDropdown from "../ui/practiceDropdown";
...
const Header = ({onMenuClick}) => {
   ...
   const learningOptions = ["grammar", "vocabulary", "speaking", "listening", "reading", "games", "courses", "practice", "challenges", "achievements"];
      ...
      <>
         <header className="border-b border-[#e5e7eb] sticky top-0 bg-white z-50">
            <div className="container mx-auto px-2 sm:px-4 flex items-center justify-between h-16">
               ...
               <button className="p-2 rounded-full hover:bg-gray-100 mr-4 items-center hover:border-transparent" onClick={handleMenuToggle} aria-label="Menú principal">
                  <Bot size={24} />
               </button>
               ...
               <div onClick={() => navigate("/")} className="flex items-center cursor-pointer">
                  <div className="p-1 rounded">
                     <img src={auroraLogo} alt="Aurora Logo" className="h-6 sm:h-7 md:h-8" />
                  </div>
               </div>
               ...
               <div className="flex items-center space-x-2 lg:hidden">
                  ...
                  <button className="p-2 rounded-full hover:bg-gray-100" onClick={() => setIsOpen(!isOpen)} aria-label="Buscar">
                     <Search size={20} />
                  </button>

                  {/* Botón para menú de usuario/app */}
                  <button className="p-2 rounded-full hover:bg-gray-100" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} aria-label="Menú de usuario">
                     <Menu size={20} />
                  </button>
               </div>
               ...
               <nav className="hidden lg:flex items-center space-x-4 xl:space-x-8">
                  {["Skills", "Grammar", "Vocabulary", "Business English", "Community", "Question Creator"].map((item) => (
                     <button
                        key={item}
                  ...
                  {showFiltered && filteredOptions.length > 0 && (
                     <div className="absolute top-full left-0 mt-1 w-full bg-white shadow-lg rounded-md z-50 max-h-60 overflow-y-auto">
                        {filteredOptions.map((option) => (
                           <button key={option} onClick={() => handleSearchOptionClick(option)} className="text-left block w-full px-4 py-2 text-sm hover:bg-gray-100">
                  ...
                  {isAuthenticated ? (
                     <div className="flex items-center space-x-2 xl:space-x-4">
                        <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="flex items-center space-x-2">
                           <User size={16} />
                           <span className="text-sm font-medium truncate max-w-[80px] xl:max-w-[150px]">{user?.username || truncateAddress(address)}</span>
                        </button>
                        <button onClick={logout} className="flex items-center space-x-1 text-sm font-medium text-gray-700 hover:text-gray-900">
                           <LogOut size={16} />
                     ...
                     <>
                        <button onClick={() => navigate("/login")} className="text-sm font-medium text-gray-700 hover:text-gray-900 hover:border-transparent">
                           Log in
                        </button>
                        <button
         ...
         {isMobileMenuOpen && (
            <div className="fixed inset-0 bg-white z-50 pt-16 overflow-y-auto lg:hidden">
               <div className="p-4">
                  <button onClick={() => setIsMobileMenuOpen(false)} className="absolute top-4 right-4 p-2 rounded-full hover:bg-gray-100" aria-label="Cerrar menú">
                     <X size={24} />
                  </button>
                  ...
                  <div className="mt-8 pt-4 border-t border-gray-200">
                     {isAuthenticated ? (
                        <div className="flex flex-col space-y-4">
                           ...
                           <button onClick={logout} className="flex items-center space-x-2 text-md font-medium text-gray-700 hover:text-gray-900 py-2">
                              <LogOut size={20} />
